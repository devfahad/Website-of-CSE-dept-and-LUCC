<div class="col-lg-2 col-md-3">
                    <ul class="top">
                        <li class="active"><a href="index.php"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
                        <li><a href="../index.php"><i class="fas fa-home"></i> Visit Website</a></li>
                        <!-- <li><a href="edit-profile.php"><i class="fas fa-user-edit"></i> Edit Profile</a></li> -->
                    </ul>
                    <ul class="bottom">
                        <li class="has-menu-item">
                            <a href="#"><i class="fas fa-blog"></i> Posts</a>
                            <ul class="dropdown_menu">
                                <li><a href="all-posts.php">All Posts</a></li>
                                <li><a href="add-new-post.php">Add New</a></li>
                            </ul>
                        </li>
                        <li class="has-menu-item">
                            <a href="#"><i class="fas fa-calendar-alt"></i> Events</a>
                            <ul class="dropdown_menu">
                                <li><a href="../events.php">View Events</a></li>
                                <li><a href="all-events.php">All Events</a></li>
                                <li><a href="add-new-event.php">Add New</a></li>
                            </ul>
                        </li>
                        <li class="has-menu-item">
                            <a href="#"><i class="fas fa-images"></i> Gallery</a>
                            <ul class="dropdown_menu">
                                <li><a href="../gallery.php">View Gallery</a></li>
                                <li><a href="all-photos.php">All Photos</a></li>
                                <li><a href="upload-photo.php">Upload</a></li>
                                <li><a href="add-new-upload-folder.php">Add New Folder</a></li>
                            </ul>
                        </li>
                        
                        <li class="has-menu-item">
                            <a href="#"><i class="fas fa-user"></i> Users</a>
                            <ul class="dropdown_menu">
                                <li><a href="all-users.php">All Users</a></li>
                                <li><a href="pending-users.php">Pending Requests</a></li>
                            </ul>
                        </li>
                        <li class="has-menu-item">
                            <a href="#"><i class="fas fa-users"></i> Committee</a>
                            <ul class="dropdown_menu">
                                <li><a href="committee-lists.php">Committee Lists</a></li>
                                <li><a href="add-new-committee.php">Add New</a></li>
                            </ul>
                        </li>
                        <li class="has-menu-item">
                            <a href="#"><i class="fas fa-money-check-alt"></i> Due/Payments</a>
                            <ul class="dropdown_menu">
                                <li><a href="view-all-user-payment-lists.php">View All</a></li>
                            </ul>
                        </li>
                        
                        <!-- <li>
                            <a href="admin-editor.php"><i class="fas fa-user-shield"></i> Admins/Editors</a>
                        </li> -->
                        
                        <li class="has-menu-item">
                            <a href="#"><i class="fas fa-users"></i>Comments</a>
                            <ul class="dropdown_menu">
                                <li><a href="comments-blog.php">Blog Comments</a></li>
                                <li><a href="comments-event.php">Event Comments</a></li>
                            </ul>
                        </li>
                        <!-- <li class="has-menu-item">
                            <a href="#"><i class="fas fa-envelope"></i> Contact</a>
                            <ul class="dropdown_menu">
                                <li><a href="contact-forms.php">Contact Forms</a></li>
                            </ul>
                        </li> -->
                    </ul>
                </div>